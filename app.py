...
    # contract_size wird geladen, aber bei BTC-Inverse nicht multipliziert:
    # OI und amount sind bereits in BTC (underlying units). contract_size ≈ 1.
...
    # ── Exposure ──
    # GEX: Positioning-Annahme Call+ / Put−
    # Einheit: modelliertes $ P&L bei +1% Spot-Move, in Mio.
    df["gex"] = np.where(
        df["type"] == "Call",
...
    # DEX: rohes Delta-Exposure aus OI (keine Call+/Put−-Umsignierung)
    # = Summe (delta × OI × spot) in Mio. $
    df["dex"] = (df["delta"] * df["open_interest"] * spot) / 1e6

    # Vega Exposure (NICHT Volga/VEX): vega × OI
...
    # Aggressor Flow (nur für gewählten Filter/Bucket)
...
        # direction = Aggressor-Seite (buy/sell), nicht zwingend "Customer long/short"
...
        ) / 1e6
        # Counterparty = Gegenposition zur Aggressor-Seite (Modellannahme)
